	...
	float p_miss) 
{
	...
	for(int i=0; i<grid.size(); i++) {
	  ...
	  for(int j=0; j<grid[0].size(); j++) {
	    char curr_grid_val = grid[i][j];
	    float curr_belief_val = beliefs[i][j];
	    
	    if (curr_grid_val == color) {
	      curr_row.push_back(curr_belief_val * p_hit);
	    } else {
	      curr_row.push_back(curr_belief_val * p_miss);
	    }
	  }

	  newGrid.push_back(curr_row);
	}
